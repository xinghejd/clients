...
import { BrowserBulkEncryptServiceImplementation } from "./browser-bulk-encrypt.service.implementation";

describe("BrowserBulkEncryptServiceImplementation", () => {
  ...
  let bulkEncryptService: BrowserBulkEncryptServiceImplementation;
  ...
  beforeEach(() => {
    ...
    bulkEncryptService = new BrowserBulkEncryptServiceImplementation(
  ...
  it("decrypts items using web workers if the chrome.offscreen API is not supported", async () => {
    ...
    await bulkEncryptService.decryptItems([], key);
  ...
  it("decrypts items using the chrome.offscreen API if it is supported", async () => {
    ...
    await bulkEncryptService.decryptItems(items, key);
    ...
    expect(BrowserApi.sendMessageWithResponse).toHaveBeenCalledWith("offscreenDecryptItemsBulk", {
      decryptRequest: expect.any(String),
    });
  });

  it("returns an empty array if the passed items are not defined", async () => {
    const result = await bulkEncryptService.decryptItems(null, key);
  ...
  it("returns an empty array if the offscreen document message returns an empty value", async () => {
    ...
    const result = await bulkEncryptService.decryptItems(items, key);
