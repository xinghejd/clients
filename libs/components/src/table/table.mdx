...
- `bitRow`: A row within the table. Used for content only.
- `bit-row-selector`: A checkbox that selects a row.
...
Pass a [CDK SelectionModel](https://material.angular.io/cdk/collections/overview) to the table's
`selectionModel` input. This will contain the selected rows.
...
<Story of={stories.SelectableRowsMultiple} />
