...
The table component supports selecting rows for bulk actions. This is enabled by using
`bitSelectAll` in the header (for a "select all" header checkbox) and `[bitSelectRow]="row"` on each
row (for each row's checkbox).
...
The selection state is managed by the `SelectionState` class and is available on
`dataSource.selection`. This can be used to programatically read and alter the selection state. For
example:

```ts
submit = async () => {
  // Get all selected items
  const items = dataSource.selection.selectedRows;

  // Execute the bulk action
  await cipherService.deleteItems(items);

  // Clear the selection
  dataSource.selection.deselectAll();
};
```

In some data sets, not all rows are selectable - for example, if the user does not have permission
to edit that item. `SelectionState.canSelect` defines a callback that is used to determine whether a
row is selectable. By default, it always returns true (i.e. assumes that all items can be selected).

```ts
this.dataSource.selection.canSelect = (item) => item.canEdit;
```

This callback is frequently called and should not be computationally expensive.
