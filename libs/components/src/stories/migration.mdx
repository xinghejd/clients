...
those. If a component isn't flexible enough or doesn't exist for your usecase, contact Will Martin.
...
and it's related CL components. Components that are fully migrated should have no reference to the
`ModalService`.

<div class="tw-bg-danger-500/10 tw-p-4">
    <span class="tw-text-danger tw-font-bold">Bad</span>
    ```ts
    this.modalService.open(DeleteAccountComponent);
    ```
