...
export class Fido2ClientService implements Fido2ClientServiceAbstraction {
  ...
  async isFido2FeatureEnabled(
    hostname: string,
    origin: string
  ): Promise<boolean> {
    const isUserLoggedIn =
      (await this.authService.getAuthStatus()) !==
      AuthenticationStatus.LoggedOut;
    if (!isUserLoggedIn) {
      return false;
    }

    const neverDomains = await firstValueFrom(
    ...
    const isOriginEqualBitwardenVault =
      origin === serverConfig.environment?.vault;
  ...
  async createCredential(
    ...
    if (!enableFido2VaultCredentials) {
      this.logService?.warning(
        `[Fido2Client] Fido2VaultCredential is not enabled`
      );
    ...
    if (
      ...
      this.logService?.warning(
        `[Fido2Client] Invalid https origin: ${params.origin}`
      );
      throw new DOMException(
    ...
    if (credTypesAndPubKeyAlgs.length === 0) {
      const requestedAlgorithms = credTypesAndPubKeyAlgs
        .map((p) => p.alg)
        .join(", ");
      this.logService?.warning(
    ...
    try {
      ...
      if (
        ...
        this.logService?.info(
          `[Fido2Client] Aborting because user requested fallback`
        );
        throw new FallbackRequestedError();
      }
    ...
    return {
      credentialId: Fido2Utils.bufferToString(
        makeCredentialResult.credentialId
      ),
      attestationObject: Fido2Utils.bufferToString(
        makeCredentialResult.attestationObject
      ),
      ...
      transports:
        params.rp.id === "google.com" ? ["internal", "usb"] : ["internal"],
  ...
  async assertCredential(
    ...
    return this.generateAssertCredentialResult(
      getAssertionResult,
      clientDataJSONBytes
    );
  }
  ...
  private async handleMediatedConditionalRequest(
    ...
    while (!getAssertionResult) {
      ...
      const result = await this.requestManager.newActiveRequest(
        tab.id,
        availableCredentials,
        abortController
      );

      if (result.type === Fido2ActiveRequestEvents.Refresh) {
        continue;
      }

      if (result.type === Fido2ActiveRequestEvents.Abort) {
        break;
      }

      params.allowedCredentialIds = [
        Fido2Utils.bufferToString(guidToRawFormat(result.credentialId)),
      ];
  ...
  private setAbortTimeout = (
    ...
    if (userVerification === "required") {
      ...
      clampedTimeout = Math.max(
        WITH_VERIFICATION.MIN,
        Math.min(timeout, WITH_VERIFICATION.MAX)
      );
    } else {
      timeout = timeout ?? NO_VERIFICATION.DEFAULT;
      clampedTimeout = Math.max(
        NO_VERIFICATION.MIN,
        Math.min(timeout, NO_VERIFICATION.MAX)
      );
    }
